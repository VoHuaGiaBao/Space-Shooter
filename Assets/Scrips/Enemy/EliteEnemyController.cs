...
{
    public float speed = 2f;            // Tốc độ di chuyển xuống.
    public float horizontalSpeed = 2f;  // Tốc độ di chuyển trái/phải.
    public float maxHealth = 100f;      // Máu tối đa của Elite Enemy.
    private float currentHealth;        // Máu hiện tại.
    public float healthBarPos = 0.6f;        // Vị trí thanh máu 
    private bool moveSideways = false;  // Kiểm tra di chuyển ngang.
    ...
    private void Start()
    {
        currentHealth = maxHealth;      // Đặt máu bắt đầu bằng giá trị tối đa.
        healthSlider.value = CalculateHealth();  // Khởi tạo thanh máu.
    ...
    private void PositionHealthBar()
    {
        ...
        healthBarPosition.y -= healthBarPos; // Đặt thanh máu phía dưới enemy.
    ...
    void Die()
    {
        ...
        // Hủy enemy sau khi bị tiêu diệt.
        Destroy(gameObject);
    }
